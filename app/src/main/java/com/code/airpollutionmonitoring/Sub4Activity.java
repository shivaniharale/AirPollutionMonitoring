...
public class Sub4Activity<name> extends AppCompatActivity {
    private static final String TAG = "Sub4Activity";
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        String value = temp.getString("field4");
                        double valueDouble=Double.parseDouble(value);
                        sum+=valueDouble;
                        average=sum/i;

                        text.append("Sr n-->"+i+"\0\0\0Date-->"+date+"\0\0\0 Time-->"+time+"\0\0\0 Entry id-->"
                                + entry + "\0\0\0Data-->" + value +"_______"+average+ "\n");
                        Log.d(TAG,"REACHED HERE TOO");

                    }
                    int length=array.length()-1;
                    currentData.append(array.getJSONObject(length).getString("field4"));
                    double data=Double.parseDouble(array.getJSONObject(length).getString("field4"));

                    //data=70;
                    if( data >= threshold)
                    {
                        message="Increased SO2 level:Perform precautionary measures";
                        addNotification(message);
                    }
                    else if (data>=limit)
                    {
                        message="Hazardous SO2 level:Evacuate immediately";
                        addNotification(message);
                        createSMS(message);
                    }
...
//        PendingIntent pendingIntent=PendingIntent.getActivity(getApplicationContext(),0,intent,0);
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            if (checkSelfPermission(Manifest.permission.SEND_SMS) == PackageManager.PERMISSION_DENIED) {
                ...
                {
                    smsManager.sendTextMessage(mobileNumbersArray(i), null, message, null, null);
                }
                 */
